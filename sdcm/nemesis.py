...
class Nemesis:  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def _after_cluster_scaleout(self, duration: int) -> None:
        InfoEvent(message=f'StartEvent - after scaleout').publish()
        time.sleep(1800)
        InfoEvent(message='FinishEvent - after scaleout').publish()

    @latency_calculator_decorator(legend="After Cluster Scalein")
    def _after_cluster_scalein(self, duration: int) -> None:
        InfoEvent(message=f'StartEvent - after scalein').publish()
        time.sleep(1800)
        InfoEvent(message='FinishEvent - after scalein').publish()
