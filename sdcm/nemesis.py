...
class Nemesis(NemesisFlags):
    ...
    def repair_nodetool_repair(self, node=None, publish_event=True):
        ...
        repair_cmd = f"cluster repair --incremental-mode {incremental_mode}" if incremental_mode is not None else "repair"
    ...
    def run_repair_on_nodes(self, nodes: list,  ignore_down_hosts=False, publish_event=True):
        ...
        if not self.cluster.params.get('use_mgmt') and not self.cluster.params.get('use_cloud_manager'):
            incremental_mode = self.cluster.params.get('incremental_repair_mode')
            repair_cmd = f"cluster repair --incremental-mode {incremental_mode}" if incremental_mode is not None else "repair -pr"
            for node in nodes:
                try:
